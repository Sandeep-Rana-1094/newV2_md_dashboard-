        ...
        <div className="bg-surface/50 backdrop-blur-sm p-4 sm:p-6 rounded-xl border border-border-default h-[400px]">
            ...
            <ResponsiveContainer width="100%" height="90%">
                <BarChart data={chartData} layout="vertical" margin={{ top: 5, right: 50, left: 30, bottom: 5 }}>
                    ...
                    <XAxis type="number" stroke="#a1aab4" tick={{ fill: '#a1aab4' }} />
                    <YAxis type="category" dataKey="displayName" stroke="#a1aab4" width={200} tick={{ fill: '#a1aab4', fontSize: 12 }} />
                    <Tooltip contentStyle={{ backgroundColor: 'rgba(22, 27, 34, 0.9)', border: '1px solid #30363d', borderRadius: '0.75rem' }} cursor={{ fill: 'rgba(45, 212, 191, 0.1)' }} itemStyle={{ color: '#e6edf3' }} labelStyle={{ color: '#e6edf3', fontWeight: 'bold' }} />
                    <Bar dataKey="quantity" name="Total Quantity" fill="#a855f7" barSize={20}>
                        <LabelList dataKey="quantity" position="right" style={{ fill: '#a1aab4', fontSize: 12 }} />
                    </Bar>
                </BarChart>
            </ResponsiveContainer>
        </div>
...
export const OrderAnalysisDashboard: React.FC = () => {
  ...
  const uniqueSegments = useMemo(() => {
    if (!orders) return [];
    return [...new Set(orders.map(order => order.segment))].sort();
  }, [orders]);
  ...
  const filteredData = useMemo(() => {
    return combinedData.filter(order => {
        // Segment filter
        if (selectedSegment && order.segment !== selectedSegment) {
            return false;
        }

        // Product Code filter
        if (productCodeFilter) {
            const searchString = productCodeFilter.toLowerCase();
            const hasMatchingProduct = order.products.some(p => 
                p.productCode.toLowerCase().includes(searchString)
            );
  ...
  }, [combinedData, selectedSegment, productCodeFilter]);

  const kpis = useMemo(() => {
    const dataToUse = filteredData;
    const totalAmount = dataToUse.reduce((acc, curr) => acc + curr.amount, 0);
    const orderCount = dataToUse.length;
    const totalQuantity = dataToUse.reduce((acc, curr) => acc + curr.products.reduce((pAcc, p) => pAcc + p.quantity, 0), 0);
    const allProducts = dataToUse.flatMap(order => order.products);
    const uniqueProductCount = new Set(allProducts.map(p => p.productCode)).size;

    return { totalAmount, orderCount, totalQuantity, uniqueProductCount };
  }, [filteredData]);
  ...
  const isFilterActive = selectedSegment !== '' || productCodeFilter !== '';
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Filter Section */}
       <div className="bg-surface/50 backdrop-blur-sm p-4 rounded-xl border border-border-default">
        <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
            <div className="flex items-center space-x-3 text-primary">
                <FilterIcon />
                <h3 className="text-md font-semibold">Filters</h3>
            </div>
             <button onClick={clearFilters} disabled={!isFilterActive} className="flex items-center justify-center px-3 py-2 text-sm font-medium text-muted bg-surface/50 border border-border-default rounded-lg hover:bg-surface disabled:opacity-50 disabled:cursor-not-allowed">
                <XCircleIcon />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
                <label htmlFor="segmentFilter" className="block text-sm font-medium text-text-secondary mb-1">Segment</label>
                <select id="segmentFilter" name="segment" value={selectedSegment} onChange={handleSegmentChange} className="w-full block px-3 py-2 bg-slate-900 border border-border-default rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary sm:text-sm">
                    <option value="">All Segments</option>
                    {uniqueSegments.map(s => <option key={s} value={s}>{s}</option>)}
                </select>
            </div>
            <div>
                <label htmlFor="productCodeFilter" className="block text-sm font-medium text-text-secondary mb-1">Product Code</label>
                <input
                    type="text"
                    ...
                    className="w-full block px-3 py-2 bg-slate-900 border border-border-default rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary sm:text-sm"
                />
            </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <KpiCard title="Total Sales" value={kpis.totalAmount.toLocaleString('en-US', { style: 'currency', currency: 'USD' })} icon={<DollarSignIcon />} color="teal" />
        <KpiCard title="Total Order number" value={kpis.orderCount.toLocaleString()} icon={<ShoppingCartIcon />} color="green" />
        <KpiCard title="Total Quantity Sold" value={kpis.totalQuantity.toLocaleString()} icon={<PackageIcon />} color="purple" />
        <KpiCard title="Unique Products" value={kpis.uniqueProductCount.toLocaleString()} icon={<CubeTransparentIcon />} color="amber" />
      </div>
      ...
      <ProductSalesTable data={filteredData} />

      <OrderDataTable data={filteredData} />
    </div>
  );
};